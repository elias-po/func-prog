...
(defn encryptRow
  ...
  (str
    ...
    (if (> (count input) (countCycle key)) (encryptRow (subs input (countCycle key)) key row) "")
    )
  )

(defn bootstrapEncryption
  [input key position]
  (if (< position key) (str (encryptRow (subs input position) key position) (bootstrapEncryption input key (+ position 1)))
                       "")
  )
